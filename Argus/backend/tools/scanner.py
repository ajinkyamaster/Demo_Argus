...
Each tool spawns a sub-agent mini-crew (cloud LLM or local Ollama dual-model)
that thinks, tests, analyses, and iterates via a feedback loop.  If the
sub-crew fails for any reason, the tool falls back to deterministic httpx
probing as a second-resort fallback.
"""
...
# Common endpoints to probe (deterministic scans try all of them)
...
def _get_session_cookie(base_url: str) -> dict | None:
    """Try to obtain a session cookie by logging in.

    Many endpoints (XSS, IDOR) require authentication. This helper tries
    known/common credentials against auth endpoints, returning cookies if
    successful.  Returns None if no cookie could be obtained.
    """
    # Credentials to try (includes known demo creds + common defaults)
    cred_list = [
        {"username": "j_sterling", "password": "Vault#2024!"},
        {"username": "s_miller", "password": "Fraud$ecure9"},
        {"username": "admin", "password": "admin"},
        {"username": "test", "password": "test"},
    ]

    for creds in cred_list:
        for ep in ["/login", "/corp/legacy-auth", "/api/login"]:
            url = f"{base_url}{ep}"
            for send_fn in [
                lambda u=url, c=creds: httpx.post(u, data=c, timeout=TIMEOUT, follow_redirects=True),
                lambda u=url, c=creds: httpx.post(u, json=c, timeout=TIMEOUT, follow_redirects=True),
            ]:
                try:
                    resp = send_fn()
                    # Check the final response and redirect chain for cookies
                    all_cookies: dict = {}
                    for hist_resp in resp.history:
                        all_cookies.update(dict(hist_resp.cookies))
...
def _make_result(tool_name: str, target: str, findings: list) -> str:
    ...
    hallucination-killer loop can read them directly without parsing the
    findings array.
    """
            ...
            for resp in _post_both_encodings(login_url, creds_bypass):
                ...
                if resp.status_code == 200 and any(m in body_lower for m in bypass_markers):
                    findings.append({
                        ...
                        "remediation": "Use parameterised queries for authentication.",
                        "cvss_score": 9.8,
                    })
...
def auth_bypass_tool(target_url: str) -> str:
    ...
    try:
        ...
        if result.get("findings"):
            return _make_result("auth_bypass", target_url, result["findings"])
    except Exception:
        pass  # Sub-crew failed — fall through to deterministic
...
def idor_probe_tool(target_url: str) -> str:
    ...
    Enumerates records by sequential ID without providing authentication
    tokens and checks whether the server returns PII for each request.
    Returns a JSON string with confirmed findings only.
    """
    try:
        ...
        for ep_pattern in _USER_ENDPOINTS:
            ...
            if collected:
                ...
                if unauthed:
                    ...
                    cvss = 9.1
                    evidence_prefix = (
                        f"Retrieved {len(unauthed)} record(s) WITHOUT authentication."
                    )
                else:
                    ...
                    cvss = 7.5
                    evidence_prefix = (
                        f"Retrieved {len(collected)} record(s) with a single "
                        f"session (no ownership check)."
                    )
...
def _deterministic_ssti_scan(target_url: str) -> str:
    ...
    param_names = ["q", "merchant", "search", "query", "name", "template"]

    cookies = _get_session_cookie(base)

    test_endpoints = [(f"{base}{ep}", "GET") for ep in _SEARCH_ENDPOINTS]
                    ...
                    if expected in body and payload not in body:
                        findings.append({
                            ...
                            "description": (
                                ...
                                f"the server ({engine}). Result '{expected}' appeared."
                            ),
                            ...
                            "evidence": (
                                ...
                                f"Engine: {engine}. Response: {body[:300]}"
                            ),
                            "remediation": (
                                "Never pass raw user input to render_template_string(). "
                    ...
                    for marker, desc in config_markers:
                        if marker in body:
                            findings.append({
                                ...
                                "title": f"SSTI — {desc}",
                                "agent": "ExploitationAgent",
                                "description": f"Template expression caused: {desc}.",
                                "endpoint": url.replace(base, ""),
                                "method": method,
                                "payload": payload,
                                "evidence": (
                                    f"Sent {payload!r}, response contains "
                                    f"'{marker}'. Response: {body[:300]}"
                                ),
                                "remediation": (
                                    "Sandbox the template engine and restrict "
                                    "built-in access."
                                ),
                                "cvss_score": 9.8,
                            })
                    ...
                    for marker in passwd_markers:
                        if marker in body:
                            findings.append({
                                ...
                                "agent": "ExploitationAgent",
                                "description": (
                                    "Path traversal sequences allow reading "
                                ...
                                "payload": payload,
                                "evidence": (
                                    f"Sent {payload!r} in param '{param_name}', "
                                    f"response contains '{marker}'. "
                                    f"Body: {body[:300]}"
                                ),
                                "remediation": (
                                    "Use an allow-list of permitted filenames. "
                                    "Strip path traversal sequences. Validate "
                                    "resolved paths with os.path.realpath()."
                                ),
                                "cvss_score": 7.5,
                            })
